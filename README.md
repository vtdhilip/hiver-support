...
An enterprise-grade, evaluation-first AI support agent built for `@AmazonHelp` on Twitter as part of the **Hiver SDE Intern Take-Home Assignment**.

The system addresses the three core requirements:
1. **Classify incoming customer tweets** into a well-defined intent taxonomy.
2. **Draft high-quality, brand-aligned replies** grounded in historical resolutions via RAG.
3. **Automate triage** by deciding whether to `Auto-Handle` or `Escalate` to a human agent with clear reasoning.
...
* **"Good"** means:
  ...
  3. **High Tone Fidelity:** Matching Amazon's polite, apologetic, and concise Twitter voice.
...
5. **Boilerplate Truncation in Historical Tweets:**
   * *Example:* Retrieved context containing truncated URLs (`https://t.co/...`).
   * *Hypothesis:* RAG context containing stale or shortened links can lead the agent to mirror generic link references.
...
3. **LLM Judge Self-Bias:** Using Gemini 1.5 Flash as an evaluator for text drafted by the same model family introduces positive bias toward its own phrasing style.
...
6. **Markdown Code-Block Stripping (`_parse_json`):** Added a defensive parser to handle edge cases where LLMs wrap valid JSON inside ` ```json ` fences.
...
11. **Caching Knowledge Base (`kb`):** Indexed the historical conversation pool once at server startup to keep runtime response times under 800ms.
